...
public class Game {
    ...
    //*********MIS VARIABLES*********//
	//Matriz que lamacena los valores originales de las casillas generadas cuando se crea el mapa
	int [][] originalType = new int [10][10];
	...
	public Game(){  
      ...
      //AQUI SE HA TERMINADO DE LLAMAR A LOS METODOS QUE CREAN EL MAPA.
        originalTypeAsigner(); //saving original status for each cell in map.
    }
    ...
    protected void InitMar(){
        //Se elige una esquina de forma randomica(25% de probabilidad para cada una de las 4 esquinas) para comenzar a pintar el mar.
  ...
  public  void changeResource(Player player, int recurso, int sumaOresta){

		  player.resources[recurso] += sumaOresta;  
 
  } 
    
  public void originalTypeAsigner(){
    	
    	for(int y = 0; y < 10; y++){
    		for(int x = 0; x < 10; x++){
    			originalType[y][x] = mapa[y][x].type;
    		}
    	}
    }
    public int getOriginalType(int x, int y){
    	return originalType[x][y];
    }
    ...
    public int getTurnNumberCurrent(){
        //System.out.println("NO IMPLEMENTADO Game: int getTurnNumberCurrent()");
        return turno+1;
    ...
    /*	int columnas = 0, filas = 0;
    	...
    	columnas = x-1; filas = y-1; if (columnas < 0 || filas < 0 || columnas > 9 || filas > 9){
    		System.out.println("La casilla de la esquina superior izquierda está fuera de limites");
    	}
    	else{mapa[columnas][filas].locked = false;}
    	//CASILLA SUPERIOR CENTRAL
    	columnas = x-1; filas = y; if (columnas < 0 || filas < 0 || columnas > 9 || filas > 9){
    		System.out.println("La casilla de la esquina superior central está fuera de limites");
    	}
    	else{mapa[columnas][filas].locked = false;}
    	//CASILLA SUPERIOR DERECHA
    	columnas = x-1; filas = y+1; if (columnas < 0 || filas < 0 || columnas > 9 || filas > 9){
    		System.out.println("La casilla de la esquina superior derecha está fuera de limites");
    	}
    	else{mapa[columnas][filas].locked = false;}
    	//CASILLA IZQUIERDA
    	columnas = x; filas = y-1; if (columnas < 0 || filas < 0 || columnas > 9 || filas > 9){
    		System.out.println("La casilla izquierda está fuera de limites");
    	}
    	else{mapa[columnas][filas].locked = false;}
    	//CASILLA SELECCIONADA
    	mapa[columnas][filas].locked = false;
    	//CASILLA DERECHA
    	columnas = x; filas = y+1; if (columnas < 0 || filas < 0 || columnas > 9 || filas > 9){
    		System.out.println("La casilla derecha está fuera de limites");
    	}
    	else{mapa[columnas][filas].locked = false;}
    	//CASILLA INFERIOR IZQUIERDA
    	columnas = x+1; filas = y-1; if (columnas < 0 || filas < 0 || columnas > 9 || filas > 9){
    		System.out.println("La casilla de la esquina inferior izquierda está fuera de limites");
    	}
    	else{mapa[columnas][filas].locked = false;}
    	//CASILLA INFERIOR CENTRAL
    	columnas = x+1; filas = y; if (columnas < 0 || filas < 0 || columnas > 9 || filas > 9){
    		System.out.println("La casilla de la esquina inferior central está fuera de limites");
    	}
    	else{mapa[columnas][filas].locked = false;}
    	//CASILLA INFERIOR DERECHA
    	columnas = x+1; filas = y+1; if (columnas < 0 || filas < 0 || columnas > 9 || filas > 9){
    		System.out.println("La casilla de la esquina inferior derecha está fuera de limites");
    	}
    	else{mapa[columnas][filas].locked = false;}*/
    }    
    ...
    public boolean canBuild(Player player, Square square, int idConstruction) {
       //System.out.println("NO IMPLEMENTADO Game: boolean canBuild(Player player, Square square, int idConstruction)");
    	...
    	case 0: //LLANURA
    		...
    		case 10://cuartel
    			if(recursos[Constants.RS_GOLD] >= 3000 && recursos[Constants.RS_WOOD] >= 3000 && square.locked == false ){return true;}else{return false;}
    		case 11://colegio
    			if(recursos[Constants.RS_GOLD] >= 1500 && recursos[Constants.RS_WOOD] >= 1500 && square.locked == false){return true;}else{return false;}
    		case 12://granja
    			if(recursos[Constants.RS_GOLD] >= 150 && recursos[Constants.RS_WOOD] >= 300 && square.locked == false){return true;}else{return false;}
    		case 16://casa
    			if(recursos[Constants.RS_GOLD] >= 100 && recursos[Constants.RS_WOOD] >= 300 && square.locked == false){return true;}else{return false;}
    		case 17://mercado
    			if(recursos[Constants.RS_GOLD] >= 100 && recursos[Constants.RS_WOOD] >= 200 && square.locked == false){return true;}else{return false;}
    		case 101:
    			if(recursos[Constants.RS_GOLD] >= 7000 && recursos[Constants.RS_WOOD] >= 7000 && square.locked == false){return true;}else{return false;}
    		case 102:
    			if(recursos[Constants.RS_GOLD] >= 100 && recursos[Constants.RS_WOOD] >= 300 && square.locked == false){return true;}else{return false;}
    		case 103:
    			if(recursos[Constants.RS_GOLD] == 300 && recursos[Constants.RS_WOOD] == 150 && square.locked == false){return true;}else{return false;}
    	...
    	case 2: //MAR
    		switch(idConstruction){
    		case 15://barco
    			if(recursos[Constants.RS_GOLD] >= 200 && recursos[Constants.RS_WOOD] >= 400 && square.locked == false){return true;}else{return false;}
    		case 103:
    			if(recursos[Constants.RS_GOLD] == 300 && recursos[Constants.RS_WOOD] == 150 && square.locked == false){return true;}else{return false;}
    		default: return false;
    		}
    ...
    public void build(Player player, Square square, int idConstruction) {
      //  System.out.println("NO IMPLEMENTADO Game: void build(Player player, Square square, int idConstruction)");
        ...
        case Constants.CO_FARM :
        	changeResource(player, Constants.RS_GOLD, -150);
        	changeResource(player, Constants.RS_WOOD, -300);
        	square.type = Constants.CO_FARM;
        	break;
        case Constants.CO_BARRACKS:
        	changeResource(player, Constants.RS_GOLD, -3000);
        	changeResource(player, Constants.RS_WOOD, -3000);
        	square.type = Constants.CO_BARRACKS;
        	break;
        case Constants.CO_BOAT:
        	changeResource(player, Constants.RS_GOLD, -200);
        	changeResource(player, Constants.RS_WOOD, -400);
        	square.type = Constants.CO_BOAT;
        	break;
        case Constants.CO_HOUSE:
        	changeResource(player, Constants.RS_GOLD, -100);
        	changeResource(player, Constants.RS_WOOD, -300);       	
        	square.type = Constants.CO_HOUSE;
            	break;
        case Constants.CO_MARKET:
        	changeResource(player, Constants.RS_GOLD, -100);
        	changeResource(player, Constants.RS_WOOD, -200);
        	square.type = Constants.CO_MARKET;
        	break;
        case Constants.CO_MINE:
        	changeResource(player, Constants.RS_GOLD, -100);
        	changeResource(player, Constants.RS_WOOD, -200);
        	square.type = Constants.CO_MINE;
        	break;
        case Constants.CO_SAWMILL:
        	changeResource(player, Constants.RS_GOLD, -100);
        	changeResource(player, Constants.RS_WOOD, -200);
	    	square.type = Constants.CO_SAWMILL;
	    	break;
        case Constants.CO_SCHOOL:
        	changeResource(player, Constants.RS_GOLD, -1500);
        	changeResource(player, Constants.RS_WOOD, -1500);
	    	square.type = Constants.CO_SCHOOL;
	    	break;
        case Constants.CO_CUSTOM_1://Costes del Coloso
        	changeResource(player, Constants.RS_GOLD, -7000);
        	changeResource(player, Constants.RS_WOOD, -7000);
        	square.type = Constants.CO_CUSTOM_1;
        	break;
        case Constants.CO_CUSTOM_2://Costes del NouCamp 
        	changeResource(player, Constants.RS_GOLD, -100);
        	changeResource(player, Constants.RS_WOOD, -300);
        	square.type = Constants.CO_CUSTOM_2;
        	break;
        case Constants.CO_CUSTOM_3://LA CARA DEL PROGRAMADOR NO TIENE COSTES
        	changeResource(player, Constants.RS_GOLD, -0);
        	changeResource(player, Constants.RS_WOOD, -0);
        	square.type = Constants.CO_CUSTOM_3;
        }
    ...
    public void destroyConstruction(int x, int y){
    	...
    	//Busca el valor original de la casilla cuando se creó el mapa.
    	mapa[x][y].type = getOriginalType(x, y); 
    ...
    public boolean isGlobalDomination(Player player){
    	...
    	for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				if(mapa[j][i].type == Constants.CO_CUSTOM_1){return true;}else{return false;}
			}
    	}
		return false;
    }    
    ...
    public int executeTurn(){
      ...
      //Se restan los costes de mantenimiento dependiendo de los edificios que se tienen y Se suman los beneficios dependiendo de los  edificios que se tienen 
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				if(mapa[j][i].type >= 10){
					aux = mapa[j][i].getCostsPerTurn(); 
					//Costes
					p1.resources[Constants.RS_GOLD] += aux[Constants.RS_GOLD]; 
					p1.resources[Constants.RS_WOOD] += aux[Constants.RS_WOOD]; 
					p1.resources[Constants.RS_FOOD] += aux[Constants.RS_FOOD]; 
					// Beneficios
					aux = mapa[j][i].getProductionPerTurn();
					p1.resources[Constants.RS_GOLD] += aux[Constants.RS_GOLD]; 
					p1.resources[Constants.RS_WOOD] += aux[Constants.RS_WOOD]; 
					p1.resources[Constants.RS_FOOD] += aux[Constants.RS_FOOD]; 
					p1.resources[Constants.RS_CULTURE] += aux[Constants.RS_CULTURE]; 
				}
			}
		}
		...
		for(int i = 0; i < 4; i++){
			if(p1.resources[i] < -100){
				return c[i];
			}
		}
		//Se suman los beneficios dependiendo de los  edificios que se tienen 
